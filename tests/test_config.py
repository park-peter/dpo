...
"""
        ...
        assert config.discovery.catalog_name == "prod"
...
class TestDiscoveryConfig:
    ...
    def test_default_values(self):
        ...
        config = DiscoveryConfig(catalog_name="test")
...
class TestProfileConfig:
    ...
    def test_inference_profile(self):
        ...
        assert config.inference_settings.problem_type == "PROBLEM_TYPE_CLASSIFICATION"
    ...
    def test_timeseries_profile(self):
        ...
        config = ProfileConfig(
            ...
            output_schema_name="monitoring",
            timeseries_settings=TimeSeriesConfig(timestamp_col="ts"),
        )
        
        assert config.profile_type == "TIMESERIES"
        assert config.timeseries_settings.timestamp_col == "ts"
    ...
    def test_baseline_table(self):
        """Test baseline table configuration."""
        config = ProfileConfig(
            profile_type="INFERENCE",
            output_schema_name="monitoring",
            baseline_table="catalog.schema.baseline",
        )
        
        assert config.baseline_table == "catalog.schema.baseline"
...
class TestOrchestratorConfig:
    ...
    def test_output_schema_validation(self):
        ...
        with pytest.raises(ValidationError):
            OrchestratorConfig(
                warehouse_id="test",
                discovery=DiscoveryConfig(catalog_name="test"),
    ...
    def test_timeseries_requires_settings(self):
        """Test TIMESERIES profile requires timeseries_settings."""
        with pytest.raises(ValidationError):
            OrchestratorConfig(
                warehouse_id="test",
                discovery=DiscoveryConfig(catalog_name="test"),
    ...
    def test_full_config(self, sample_config):
        ...
        assert sample_config.discovery.catalog_name == "test_catalog"
