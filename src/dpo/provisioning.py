...
class ProfileProvisioner:
    ...
    - Baseline table support for drift comparison
    """
    ...
    def __init__(
        ...
        self.config = config
        self.catalog = config.discovery.catalog_name
    ...
    def _build_data_profiling_config(
        ...
        """Build DataProfilingConfig based on profile_type.
        ...
        granularity = self.GRANULARITY_MAP.get(
            self.config.profile_defaults.granularity,
        ...
        baseline_table = self.config.profile_defaults.baseline_table
        ...
        if profile_type == "SNAPSHOT":
            ...
            ts_settings = self.config.profile_defaults.timeseries_settings
            return DataProfilingConfig(
                output_schema_id=schema_id,
                assets_dir=assets_dir,
                skip_builtin_dashboard=skip_builtin_dashboard,
                time_series=TimeSeriesConfig(
                    timestamp_column=ts_settings.timestamp_col,
            ...
            inference_settings = self.config.profile_defaults.inference_settings
            ...
            return DataProfilingConfig(
                ...
                inference_log=InferenceLogConfig(
                    ...
                    prediction_column=inference_settings.prediction_col,
                    timestamp_column=inference_settings.timestamp_col,
                    label_column=inference_settings.label_col,
                    model_id_column=inference_settings.model_id_col,
                    granularities=[granularity],
                ),
                slicing_exprs=slicing_exprs if slicing_exprs else None,
                baseline_table_name=baseline_table,
    ...
    def _build_config_dict(self, table: DiscoveredTable) -> dict:
        ...
        base_config = {
            ...
            "granularity": self.config.profile_defaults.granularity,
            "slicing_columns": self.config.profile_defaults.slicing_columns,
            "baseline_table": self.config.profile_defaults.baseline_table,
        }
        ...
        if profile_type == "INFERENCE":
            ...
            base_config.update({
                ...
                "prediction_col": inference_settings.prediction_col,
                "label_col": inference_settings.label_col,
                "timestamp_col": inference_settings.timestamp_col,
            })
        elif profile_type == "TIMESERIES":
            ts_settings = self.config.profile_defaults.timeseries_settings
            base_config["timestamp_col"] = ts_settings.timestamp_col

        return base_config

    def _resolve_problem_type(self, table: DiscoveredTable) -> InferenceProblemType:
        """Resolve problem type with tag override, config default, or auto-detect."""
        ...
        inference_settings = self.config.profile_defaults.inference_settings
        if inference_settings and inference_settings.problem_type:
            if "REGRESSION" in inference_settings.problem_type.upper():
                return InferenceProblemType.INFERENCE_PROBLEM_TYPE_REGRESSION
            return InferenceProblemType.INFERENCE_PROBLEM_TYPE_CLASSIFICATION

        # Auto-detect from label column type
        if inference_settings and inference_settings.label_col:
            for col in table.columns:
                if col.name.lower() == inference_settings.label_col.lower():
    ...
    def _validate_slicing_columns(self, table: DiscoveredTable) -> List[str]:
        ...
        """
        candidate_columns = self.config.profile_defaults.slicing_columns
